...
public class ScrapperPage extends BasePage {
...
//	String pgcountDString=ConfigReader.pcoshref();
	...
	String backurl,recipename,recipeid,foodcategorystr,recipecategorystr,ingredientstr,recipeurl,allNutrientvaluestr,toaddstr;
	 public  void getrecipecard(String pgcountDString, String targeted) throws InterruptedException, IOException
	 {
//		 //Getting 1st Page.
			...
			{  
			   ...
			   recipeurl=driver.getCurrentUrl();
				...
				{  	ArrayList<String>  recipeinfo= new ArrayList<String>();
					...
					recipeinfo=Baseutils.additem(recipeid, recipename, foodcategorystr,recipecategorystr,ingredientstr,Preparationtime.getText(),Cookingtime.getText(),Preparationmethod.getText(),allNutrientvaluestr,targeted,recipeurl,toaddstr);
					recipeslst.add(recipeinfo);
			/*---------------------------------------------------------*/		
					/* Add allergie recipe to allergielist*/
					allergieslst=Baseutils.readExcelEliminate(ConfigReader.getInputExcel(), 6);
	  			    resultallergies=Baseutils.FilterOperation(ingredientList, allergieslst);
				    if(resultallergies.size()>0)
				    {  	allergierecipeinfo=Baseutils.additem(recipeid, recipename, foodcategorystr,recipecategorystr,ingredientstr,Preparationtime.getText(),Cookingtime.getText(),Preparationmethod.getText(),allNutrientvaluestr,targeted,recipeurl,toaddstr);
